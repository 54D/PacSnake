...
Snake::Snake(double row, double col, double speed, Direction headingDirection, int max_health, int length) :
	...
	for (int i = 0; i < length; i++) {
		switch (headingDirection) {
            case Direction::NORTH: 	temp_row += 10.0;	break;
            case Direction::EAST:	temp_col -= 10.0;	break;
            case Direction::SOUTH:	temp_row -= 10.0;	break;
            case Direction::WEST:	temp_col += 10.0;	break;
		}
...
void Snake::move_forward() {
	...
	while (currentSnakeBody != nullptr) {
		...
		switch(currentSnakeBody->headingDirection) {
            case Direction::NORTH:	qDebug() << "UP"; currentSnakeBody->set_relative_coordinate(-1 * currentSnakeBody->speed, 0.0);	 break;
            case Direction::EAST:	qDebug() << "RIGHT"; currentSnakeBody->set_relative_coordinate(0.0, currentSnakeBody->speed);    break;
            case Direction::SOUTH:	qDebug() << "DOWN"; currentSnakeBody->set_relative_coordinate(currentSnakeBody->speed, 0.0);     break;
            case Direction::WEST:	qDebug() << "LEFT"; currentSnakeBody->set_relative_coordinate(0.0, -1 * currentSnakeBody->speed);break;
		}
        currentSnakeBody->setPos(currentSnakeBody->get_col(),currentSnakeBody->get_row());
